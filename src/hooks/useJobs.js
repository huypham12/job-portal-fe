...
export function useMyJobs(initialFilters = {}) {
  ...
  const fetchJobs = useCallback(async (newFilters = {}) => {
      ...
      if (Array.isArray(response)) {
        ...
        if (response.pagination && typeof response.pagination === 'object') {
          paginationData = {
            ...
            total_pages: response.pagination.total_pages || response.pagination.totalPages || 0
          }
        }
      }
  ...
  useEffect(() => {
    fetchJobs()
  }, [filters.page, filters.limit, filters.status, filters.sort_by, filters.sort_order])
...
export function useJobCreate() {
  ...
  const createJob = useCallback(async (jobData) => {
    ...
    try {
      ...
      const errorMessage = err.data?.message || err.message || 'Failed to create job'
...
export function useJobUpdate(jobId) {
  ...
  const updateJob = useCallback(async (jobData) => {
    ...
    try {
      ...
      const errorMessage = err.data?.message || err.message || 'Failed to update job'
      setError(errorMessage)
      throw err
    } finally {
      setLoading(false)
      ...
      const normalizeJob = (j) => {
        ...
        try {
          if (Array.isArray(copy.job_skills)) {
            ...
            copy.skills = copy.job_skills.map(js => js.skills).filter(Boolean)
          }
          if (Array.isArray(copy.job_categories)) {
            copy.category_ids = copy.job_categories.map(jc => jc.category_id)
            copy.categories = copy.job_categories.map(jc => jc.categories).filter(Boolean)
          }
        } catch (e) {
...
export function useBulkJobActions() {
  ...
  const performBulkAction = useCallback(async (action, jobIds) => {
    ...
    try {
      ...
      const errorMessage = err.data?.message || err.message || `Failed to ${action} jobs`
      setError(errorMessage)
      throw err
    } finally {
      setLoading(false)
  ...
  const bulkClose = useCallback((jobIds) => performBulkAction('close', jobIds), [performBulkAction])
  const bulkDelete = useCallback((jobIds) => performBulkAction('delete', jobIds), [performBulkAction])
  const bulkPublish = useCallback((jobIds) => performBulkAction('publish', jobIds), [performBulkAction])
  ...
  const bulkExtendExpiry = useCallback(async (jobIds, newExpiresAt) => {
    ...
    try {
      ...
      const errorMessage = err.data?.message || err.message || 'Failed to extend job expiry'
      setError(errorMessage)
      throw err
    } finally {
      setLoading(false)
...
export function useSkills(searchTerm = '', category = '', limit = 50) {
  ...
  const searchSkills = useCallback(async (search = searchTerm, cat = category) => {
    setLoading(true)
    setError(null)
...
export function useJobsSearch(initialFilters = {}) {
  ...
  // Auto-search when filters change (except q which is debounced)
  useEffect(() => {
    const shouldSearch = searchState.location ||
                        searchState.jobType ||
                        searchState.experienceLevel ||
                        (searchState.skills && searchState.skills.length > 0) ||
                        searchState.salaryMin ||
                        searchState.salaryMax ||
                        (searchState.jobCategories && searchState.jobCategories.length > 0) ||
                        (searchState.jobBenefits && searchState.jobBenefits.length > 0) ||
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (searchState.jobCategories && searchState.jobCategories.length > 0) count++
    if (searchState.jobBenefits && searchState.jobBenefits.length > 0) count++
