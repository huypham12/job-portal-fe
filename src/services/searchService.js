...
function buildSearchQueryString(filters = {}) {
  ...
  if (
    ...
    Array.isArray(filters.jobCategories) &&
  ...
  if (
    filters.jobBenefits &&
    Array.isArray(filters.jobBenefits) &&
  ...
  // Remote percentage
  if (
    filters.remotePercentageMin !== undefined &&
    filters.remotePercentageMin !== null
  ) {
  ...
  if (filters.userLocationId)
    params.append("userLocationId", filters.userLocationId);
...
export const searchService = {
  ...
  getSuggestions: (filters = {}) => {
    ...
    if (filters.userLocation)
      params.append("userLocation", filters.userLocation);
    if (filters.userExperienceLevel !== undefined) {
      params.append("userExperienceLevel", String(filters.userExperienceLevel));
    }
    if (filters.userSkills && Array.isArray(filters.userSkills)) {
  ...
  getRecentlyViewedJobs: (filters = {}) => {
    ...
    const query = params.toString();
    return api.get(
      `/api/search/jobs/recently-viewed${query ? `?${query}` : ""}`
    );
  },
  ...
  getSearchHistory: async (options = {}) => {
    ...
    const response = await api.get(
      `/api/search/history${query ? `?${query}` : ""}`
    );
    // Backend returns { success: true, data: { history, pagination } }
    // Unwrap to get just { history, pagination }
